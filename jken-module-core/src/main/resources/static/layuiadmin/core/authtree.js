...
layui.define(['jquery', 'form'], function (exports) {
    ...
    var obj = {
        ...
        render: function (dst, trees, opt) {
            ...
            opt.authKey = opt.authKey ? opt.authKey : 'auth';
        ...
        renderAuth: function (tree, dept, opt) {
            ...
            layui.each(tree, function (index, item) {
                ...
                var isAuth = _this._getStatusByDynamicKey(item, opt.authKey, opt.valueKey);

                var rowFlag = isAuth;//!hasChild && opt.collapseLeafNode;
                if (rowFlag) {
            ...
            for (var index in list) {
                if (list.hasOwnProperty(index)) {
                    ...
                    if (typeof item[opt.parentKey] !== 'undefined' && item[opt.parentKey] === startPid) {
                        ...
                        node['auth'] = this._getStatusByDynamicKey(item, opt.authKey, opt.authKey);
                        child.push(node);
                    }
                }
            }
        ...
        _treeToSelect: function (tree, currentDept, opt) {
            ...
            for (var index in tree) {
                ...
                ansList.push({
                    ...
                    auth: this._getStatusByDynamicKey(item, opt.authKey, opt.valueKey)
                });
                // 添加子节点
                if (child_flag) {
